...
export function DeleteAlert({
  ...
  useEffect(() => {
    if (openAlert) {
      ...
      if (action === 'deleteIngredient') setModalTitle('ingrediente');
  ...
  const onSubmit = async () => {
    ...
    const query = { data: { PK: element.PK } };

    const deleteRq = IngredientService.deleteIngredient(query);
  ...
  return (
    ...
    >
      {element && (
        <Typography sx={{ m: 0 }}>
          ¿Seguro(a) que desea eliminar <b>{element.name}</b>? Esta acción es irreversible
        </Typography>
      )}
